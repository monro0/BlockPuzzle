document.addEventListener('DOMContentLoaded', () => {
    ...
    // <<-- ГЛАВНОЕ ИСПРАВЛЕНИЕ: Единственная функция для отрисовки доски
    function updateBoard() {
        for (let r = 0; r < BOARD_SIZE; r++) {
            for (let c = 0; c < BOARD_SIZE; c++) {
                const cell = boardElement.children[r * BOARD_SIZE + c];
                const colorClass = board[r][c];
                cell.className = 'cell'; // Сначала сбрасываем
                if (colorClass) {
                    cell.classList.add(colorClass); // Потом добавляем цвет, если он есть
                }
            }
        }
    }
