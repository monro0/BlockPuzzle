document.addEventListener('DOMContentLoaded', () => {
    ...
    const PREVIEW_BLOCK_SIZE = 22; // <<-- ИЗМЕНЕНИЕ: Константа для размера блока предпросмотра
    ...
    let board = [];
    let currentPieces = [];
    // <<-- УДАЛЕНО: selectedPiece больше не нужен
    ...
    } catch (e) { console.log("Не в среде Telegram."); }
    ...
    function drawCurrentPieces() {
        ...
        currentPieces.forEach(piece => {
            ...
            grid.style.gridTemplateColumns = `repeat(${piece.shape[0].length}, ${PREVIEW_BLOCK_SIZE}px)`;
            grid.style.pointerEvents = 'none';
            for (let r = 0; r < piece.shape.length; r++) {
                for (let c = 0; c < piece.shape[r].length; c++) {
                    const block = document.createElement('div');
                    block.classList.add('preview-block'); // Используем класс вместо инлайн-стилей
                    if (piece.shape[r][c]) {
    ...
    // <<-- УДАЛЕНО: Функция rotatePiece() больше не нужна
    ...
    // --- Кнопки и выбор ---
    // <<-- УДАЛЕНО: Вся логика кликов для выбора и вращения
